...
create_tfvars() {
    ...
    # Clone repo to create the tfvars file
    WORK_DIR="/tmp/gitops-metal-foundry-$$"
    git clone --depth 1 "https://github.com/${GITHUB_OWNER}/${GITHUB_REPO}.git" "$WORK_DIR" 2>/dev/null || {
        log_error "Could not clone repo. Make sure it exists and is accessible."
        return 1
    }
...
EOF
    ...
    echo ""
    echo "  1. Review and commit the terraform.tfvars file:"
    echo ""
    echo "     cd $WORK_DIR"
    echo "     cat terraform/terraform.tfvars"
    echo "     git add terraform/terraform.tfvars"
    echo "     git commit -m 'feat: add OCI configuration'"
    echo "     git push"
    echo ""
    echo "  2. Create a PR or push to main to trigger Terraform:"
    echo "     - PR → terraform plan (shows what will be created)"
    echo "     - Merge to main → terraform apply (creates infrastructure)"
    echo ""
    echo "  3. After infrastructure is created:"
    echo "     - Get VM IP from GitHub Actions output"
